...
def drop_missing_values(df_1, df_2, df_3, limit, output_file_path):
    ...
    if df_3 is not None:
        missing_data_3 = find_missing_values(df_3)

        df_3 = df_3.drop(missing_data_3[missing_data_3['Percent'] > limit].index, axis=1)

        delete_row_cols_df_3 = missing_data_3[
            (missing_data_3['Percent'] <= limit) & (missing_data_3['Percent'] > 0)].index
        for row in delete_row_cols_df_3:
            df_3 = df_3.drop(df_3.loc[df_3[row].isnull()].index)

        # control if there are no missing values left
        print("Missing values left: " + str(df_3.isnull().sum().sum()))
    ...
    if output_file_path is not None:
        ...
        if missing_data_1 is not None:
            missing_data_1.to_excel(writer, 'airlines')
        if missing_data_2 is not None:
            missing_data_2.to_excel(writer, 'airport')
        if missing_data_3 is not None:
            missing_data_3.to_excel(writer, 'flights')
        writer.save()
